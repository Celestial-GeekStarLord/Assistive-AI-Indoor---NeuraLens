...
Supports voice input and camera capture with Gemini AI integration
"""
...
class AccessibilityAssistant:
    def __init__(self, api_key):
        ...
        self.speak("Accessibility Assistant is ready. Press V for voice input, C for camera capture, or Q to quit.")
    ...
    def send_text_to_gemini(self, text):
        ...
        try:
            self.speak("Sending your query to Gemini AI...")
            print(f"Sending to Gemini: {text}")
            
            # Create prompt for better context
            prompt = f"You are assisting a visually impaired person. Please provide clear, concise, and helpful responses. User query: {text}"
    ...
    def send_image_to_gemini(self, image, context=None):
        ...
        try:
            ...
            if context:
                prompt = f"You are assisting a visually impaired person. Describe this image in detail, focusing on: {context}"
            else:
                prompt = "You are assisting a visually impaired person. Please describe this image in detail, including objects, people, text, colors, and any relevant context. Be clear and descriptive."
    ...
    def run(self):
        ...
        print("="*60)
        print("Press 'V' - Voice Input (Microphone)")
        print("Press 'C' - Camera Capture")
    ...
    def run_alternative_input(self):
        ...
        while self.is_running:
            print("\n" + "="*60)
            print("Enter command:")
            print("  'v' or 'voice' - Voice Input")
            print("  'c' or 'camera' - Camera Capture")
